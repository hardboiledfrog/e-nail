...
PID myPID(&pv, &heater, &setpoint,10,0.001,5, DIRECT);
...
void setup() {
  ...
  Serial.begin(230400);
 // set PID to range between 0 and 50% window size
  myPID.SetOutputLimits(0, (WindowSize * 0.5));
  windowStartTime = millis();
  setpoint = dabTemp; // init set point

  // set up timer1 for 100ms interrupts 
...
void driveOutput() {
     ...
     if((heater > 100) && (heater > now - windowStartTime)) {
